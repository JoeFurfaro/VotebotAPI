...
logging.basicConfig(filename=str(pathlib.Path(__file__).parent.absolute()) + "/server.log", level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
...
parser.add_argument("-p", "--permission", help="Identifies a permissions string", nargs=1)
parser.add_argument("-P", "--plugin", help="Identifies a plugin name", nargs=1)
...
if args.action == "runserver":
    ...
    if args.list:
        ...
        active_plugin = plugins.find(args.plugin[0])
        if active_plugin:
            if args.edit == None:
                if not args.quiet:
                ...
                if args.edit == []:
                    print("[SERVER] You did not supply any arguments!")
                else:
                    active_plugin.process_command(args.edit)
        else:
            if not args.quiet:
                print("The plugin '" + args.plugin[0] + "' is not installed")
    else:
        if not args.quiet:
